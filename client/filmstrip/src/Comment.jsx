...
const Comment = prop => {
    ...
    const handleDeleteClick = () => {
        axios.delete(`https://localhost:3001/movie/${prop.movieId}`, {params: {commentId: prop.commentId}})
        .then(response => {
    ...
    const handleSendCommentClick = () => {
        if(comment!==''){
          ...
          setComment('');
          axios.put(`https://localhost:3001/movie/${prop.movieId}`, {commentId: prop.commentId, newComment: comment})
            .then(response => {
              window.location.reload();
            })
            .catch(err => {
